...
export async function POST(req: NextRequest) {
    try {
        const { googleUserId, type, email, userId ,currency} = await req.json();

        // Relay to Java Backend
        const backendRes = await fetch('https://api.removermarca.com/prod-api/paypal/smart/create-order', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                googleUserId,
                type,
                project: "removermarca",
currency
            }),
        });
